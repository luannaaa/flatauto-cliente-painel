...
import { X, Building2, UserRound, MapPin, Package, Truck, CalendarDays, Clock, DollarSign, ClipboardList, Navigation, Save } from "lucide-react"
      ...
      <section className={`w-full max-w-[980px] overflow-hidden rounded-[30px] border ${ui.card}`}>
        <header className={`flex items-center justify-between border-b px-6 py-5 ${ui.linha}`}>
          <div>
            ...
            <p className={`mt-1 text-sm ${ui.textoFraco}`}>Cadastre uma nova entrega para a empresa acompanhar.</p>
          </div>
          <button onClick={fechar} className={`rounded-xl border p-3 ${ui.card2}`}><X size={22} /></button>
        </header>
        ...
        <div className="grid gap-5 p-6 lg:grid-cols-[1.4fr_0.8fr]">
          <div className="space-y-5">
            ...
            <div>
              <label className={`mb-2 block text-sm font-bold ${ui.textoFraco}`}>Observações da entrega</label>
              <div className={`flex min-h-[110px] gap-3 rounded-2xl border px-4 py-3 ${ui.card2}`}>
                ...
                <textarea placeholder="Ex: carga frágil, precisa de ajudante, horário combinado..." className="min-h-[85px] flex-1 resize-none bg-transparent text-sm outline-none" />
              </div>
            </div>
          </div>
          ...
          <aside className={`rounded-[24px] border p-5 ${ui.card2}`}>
            ...
            <p className={`mt-2 text-sm ${ui.textoFraco}`}>Essa tela ainda está no visual. Depois vamos ligar no backend para salvar no banco.</p>
            ...
            <button className="mt-6 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#ffc400] font-black text-black"><Save size={19} />Salvar Entrega</button>
            <button onClick={fechar} className={`mt-3 h-12 w-full rounded-xl border font-bold ${ui.card2}`}>Cancelar</button>
          </aside>
        </div>
      </section>
    </div>
...
function ResumoLinha({ label, valor }: any) {
  return <div className="flex items-center justify-between gap-3 text-sm"><span className="opacity-60">{label}</span><strong className="text-right">{valor}</strong></div>
}
