    ...
    <main className={`min-h-screen px-6 py-6 ${ui.pagina}`}>
      <header className="mb-6 flex items-center justify-between">
        ...
        <button
          ...
          className="flex h-12 items-center gap-2 rounded-xl bg-[#ffc400] px-5 font-black text-black shadow"
        >
          <Plus size={20} />
          Nova Entrega
        </button>
            ...
            <tbody>
              {entregas.map((entrega) => (
                <tr key={entrega.id} className={`border-b ${ui.linha}`}>
                  ...
                  <td><IconText icon={<Truck size={17} className={ui.iconeEscuro} />} text={entrega.veiculo} /></td>
                  <td><IconText icon={<CalendarDays size={17} className={ui.iconeEscuro} />} text={`${entrega.data} • ${entrega.horario}`} /></td>
      ...
      {modalAberto && <ModalNovaEntrega ui={ui} claro={claro} fechar={() => setModalAberto(false)} />}
    </main>
  )
}

function ModalNovaEntrega({ ui, claro, fechar }: any) {
  return (
