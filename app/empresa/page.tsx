...
const crmColunas = [
  { nome: "Novos Leads", total: 8, cor: "border-[#d4af37]/35 bg-[#d4af37]/8", itens: [{ nome: "Empresa ABC", local: "São Paulo - SP", valor: "R$ 12.500" }, { nome: "Indústria Lima", local: "Campinas - SP", valor: "R$ 8.750" }] },
...
export default function PainelEmpresa() {
  ...
  const ui = useMemo(() => ({
    ...
    sidebar: claro ? "bg-white border-[#e8dcc2] text-[#111111] shadow-[10px_0_35px_rgba(15,23,42,0.06)]" : "bg-[#030506]/94 border-white/10 text-white",
    menuInativo: claro ? "text-[#111111]/70 hover:bg-[#d4af37]/10 hover:text-[#111111]" : "text-white/75 hover:bg-white/[0.06] hover:text-[#d4af37]",
    card: claro ? "border-[#e8dcc2] bg-white shadow-[0_18px_50px_rgba(15,23,42,0.08)]" : "border-white/10 bg-[#10171b]/88 shadow-[0_18px_45px_rgba(0,0,0,0.30)]",
    card2: claro ? "border-[#e8dcc2] bg-[#fbfaf7]" : "border-white/10 bg-white/[0.045]",
    textoFraco: claro ? "text-[#111111]/58" : "text-white/58",
  ...
  return (
    <main className={`min-h-screen ${ui.pagina}`}>
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        ...
        <div className="absolute -left-44 top-44 h-[480px] w-[480px] rounded-full bg-[#d4af37]/10 blur-[150px]" />
        <div className="absolute bottom-[-150px] right-[-70px] h-[460px] w-[460px] rounded-full bg-[#d4af37]/8 blur-[160px]" />
      </div>
      ...
      <section className="relative min-h-screen px-4 pb-28 pt-5 xl:hidden">
        <div className="mx-auto max-w-[430px]">
          ...
          <div className="mt-7">
            <h1 className={`text-[25px] font-black leading-tight ${claro ? "text-[#d4af37]" : "text-white"}`}>Olá, Transportes Silva LTDA 👋</h1>
          ...
          <div className="mt-5 grid grid-cols-[1fr_auto] gap-3">
            ...
            <button onClick={() => setModalAberto(true)} className="flex h-12 items-center gap-1 rounded-xl bg-[#d4af37] px-4 font-black text-white"><Plus size={18} />Nova</button>
          </div>
        ...
        <nav className={`fixed bottom-0 left-0 right-0 border-t px-5 py-3 backdrop-blur-2xl ${ui.card2}`}>
          <div className="mx-auto flex max-w-[430px] items-center justify-between">
            ...
            <Link href="/empresa/entregas" className="flex h-16 w-16 items-center justify-center rounded-full bg-[#d4af37] text-white shadow-[0_0_45px_rgba(255,196,0,0.45)]"><Plus size={34} strokeWidth={2.8} /></Link>
...
function Topo({ periodo, setPeriodo, ui, abrirModal }: any) {
  return (
    <header className="flex items-start justify-between gap-6">
      <div>
        <h1 className="text-[36px] font-black leading-tight tracking-[-0.035em] text-[#d4af37]">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          onClick={abrirModal}
          className="flex h-12 items-center gap-2 rounded-xl bg-[#d4af37] px-7 font-black text-white shadow-[0_0_28px_rgba(255,196,0,0.38)]"
        >
          <Plus size={18} />
          Nova Entrega
        </button>
    ...
    <aside className={`border-r px-5 py-7 backdrop-blur-xl ${ui.sidebar}`}>
        ...
        {menu.map((item) => (
          <Link
            ...
            className={`group flex min-h-[52px] w-full items-center gap-4 rounded-2xl px-4 text-left font-bold transition ${
              item.ativo
                ? "bg-[#d4af37] text-white shadow-[0_0_28px_rgba(212,175,55,0.22)]"
                : ui.menuInativo
            }`}
          >
            ...
            {item.badge && (
              <span className="rounded-md border border-[#d4af37]/70 px-2 py-0.5 text-[10px] text-[#d4af37]">
                {item.badge}
              </span>
            )}
          </Link>
      ...
      <div className={`mt-8 rounded-2xl border p-4 ${ui.card}`}>
        <div className="flex items-center gap-3">
          <div className="text-[#d4af37]"><Package size={35} /></div>
          <div>
      ...
      <div className={`mt-8 flex items-center justify-between rounded-full border p-2 ${ui.card2}`}>
        ...
        <button onClick={() => setTema("dark")} className={`flex h-9 w-9 items-center justify-center rounded-full ${tema === "dark" ? "bg-[#d4af37] text-white" : ""}`}>☀</button>
        <button onClick={() => setTema("light")} className={`flex h-9 w-9 items-center justify-center rounded-full ${tema === "light" ? "bg-[#d4af37] text-white" : ""}`}>◐</button>
      </div>
    </aside>
  )
}
...
function Card({ titulo, acao, children, ui }: any) {
  return <section className={`relative overflow-hidden rounded-[26px] border p-5 backdrop-blur-xl ${ui.card}`}><div className="pointer-events-none absolute -right-10 -top-10 h-28 w-28 rounded-full bg-[#d4af37]/10 blur-[38px]" /><div className="relative mb-5 flex items-center justify-between"><h2 className="font-black">{titulo}</h2>{acao && <button className={`rounded-lg border px-3 py-2 text-sm text-[#d4af37] ${ui.card2}`}>{acao}</button>}</div><div className="relative">{children}</div></section>
}
...
function NavMobile({ icon, texto, ativo, claro, href = "/empresa" }: any) {
  return (
    <Link
      href={href}
      className={`flex flex-col items-center gap-1 ${ativo ? "text-[#d4af37]" : claro ? "text-black/55" : "text-white/55"}`}
    >
