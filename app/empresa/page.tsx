...
const linksMenu = [
  ...
  ["Motoristas", "/empresa/motoristas"],
  ["Veículos", "/empresa/veiculos"],
  ["Clientes", "/empresa/clientes"],
  ["CRM", "/empresa/crm"],
  ["Financeiro", "/empresa/financeiro"],
  ["Configurações", "/empresa/configuracoes"],
]
...
export default function VeiculosPage() {
  ...
  const [menuVeiculoAberto, setMenuVeiculoAberto] = useState<number | null>(null)
  const [veiculosLista, setVeiculosLista] = useState<Veiculo[]>(veiculosIniciais)
    ...
    <main className={`min-h-screen px-4 py-5 sm:px-6 lg:px-10 ${ui.pagina}`}>
      {menuMobileAberto && (
        <div className="fixed inset-0 z-[999] md:hidden">
          ...
          <aside className={`absolute left-0 top-0 h-full w-[84%] max-w-[330px] border-r p-5 ${ui.card}`}>
            <div className="mb-6 flex items-center justify-between">
              <div>
                <strong className="text-xl font-black text-[#d4af37]">FLATAUTO</strong>
                <p className={`text-xs font-bold ${ui.textoFraco}`}>EMPRESA</p>
              </div>

              <button
                onClick={() => setMenuMobileAberto(false)}
                className={`flex h-10 w-10 items-center justify-center rounded-xl border ${ui.card2}`}
              >
                <X size={20} />
              </button>
            </div>
            ...
            <nav className="space-y-2">
              {linksMenu.map(([nome, link]) => (
                <a
                  key={nome}
                  href={link}
                  onClick={() => setMenuMobileAberto(false)}
                  className={`block rounded-xl border px-4 py-3 text-sm font-black ${ui.card2}`}
                >
                  {nome}
                </a>
              ))}
        ...
        <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            <button
              ...
              className={`mt-1 flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border md:hidden ${ui.card2}`}
            >
              <Menu size={25} />
            </button>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {veiculosLista.map((veiculo) => (
              <CardVeiculo
                ...
                veiculo={veiculo}
                ui={ui}
                menuAberto={menuVeiculoAberto}
                setMenuAberto={setMenuVeiculoAberto}
          ...
          <div className="mt-4 flex items-center justify-between gap-3">
            ...
            <div className="relative">
              ...
              {menuAberto === veiculo.id && (
                <div className={`absolute bottom-11 right-0 z-50 w-40 rounded-xl border p-2 ${ui.card}`}>
                  <button
                    onClick={() => excluirVeiculo(veiculo.id)}
                    className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-black text-red-500 hover:bg-red-500/10"
                  >
                    <Trash2 size={16} />
                    Excluir
                  </button>
